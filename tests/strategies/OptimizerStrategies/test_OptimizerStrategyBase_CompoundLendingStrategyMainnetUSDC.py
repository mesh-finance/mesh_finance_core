...
@pytest.fixture
def compound_strat(CompoundLendingStrategyMainnetUSDC, optimizer_strat, accounts):
    return CompoundLendingStrategyMainnetUSDC.deploy(optimizer_strat, {'from': accounts[0]})

@pytest.mark.require_network("mainnet-fork", "hardhat-fork")
def test_add_compound_strategy_to_optimizer(compound_strat, optimizer_strat, accounts):
    tx = optimizer_strat.addStrategy(compound_strat, {'from': accounts[1]})

    assert optimizer_strat.getStrategies()[0] == ["CompoundLendingStrategyMainnetUSDC", compound_strat, 0, compound_strat.apr()]
    assert tx.events["StrategyAddedOptimizer"].values() == [compound_strat]

@pytest.mark.require_network("mainnet-fork", "hardhat-fork")
def test_add_same_strategy_to_optimizer(compound_strat, optimizer_strat, accounts):
    tx = optimizer_strat.addStrategy(compound_strat, {'from': accounts[1]})

    with brownie.reverts("The strategy is already added in this optimizer"):
        optimizer_strat.addStrategy(compound_strat, {'from': accounts[1]})

@pytest.fixture
def fund_through_proxy_with_compound_strategy_and_deposit(fund_through_proxy_usdc, optimizer_strat, compound_strat, accounts, usdc, test_usdc_account):
...
@pytest.mark.require_network("mainnet-fork", "hardhat-fork")
def test_set_invest_activate_with_random_address(fund_through_proxy_with_compound_strategy_and_deposit, optimizer_strat, accounts):
...
@pytest.mark.require_network("mainnet-fork", "hardhat-fork")
def test_fund_with_compound_strategy_hardwork_with_optimizer_invest_activate_false(fund_through_proxy_with_compound_strategy_and_deposit, optimizer_strat, compound_strat, usdc, accounts, interface):
    deposited_amount = 1000 * (10 ** usdc.decimals())
    required_fund = fund_through_proxy_with_compound_strategy_and_deposit
    usdc_balance_in_compound_before = compound_strat.investedUnderlyingBalance()
    ...
    usdc_balance_in_compound_after = compound_strat.investedUnderlyingBalance()
    usdc_balance_in_optimizer_after = interface.IERC20(usdc).balanceOf(optimizer_strat)
    assert usdc_balance_in_compound_before == usdc_balance_in_compound_after == 0
...
def test_deposit_and_hard_work_with_compound_strategy(fund_through_proxy_with_compound_strategy_and_deposit, optimizer_strat, compound_strat, usdc, accounts):
    required_fund = fund_through_proxy_with_compound_strategy_and_deposit
...
@pytest.fixture
def fund_with_deposit_and_hard_work_with_compound_strategy(fund_through_proxy_with_compound_strategy_and_deposit, optimizer_strat, compound_strat, usdc, accounts):
    required_fund = fund_through_proxy_with_compound_strategy_and_deposit
    required_fund.doHardWork({'from': accounts[1]})
...
def test_fund_with_compound_strategy_and_profit1_strategy_hardwork(fund_with_deposit_and_hard_work_with_compound_strategy, optimizer_strat, compound_strat, profitstrat_1_optimizer, accounts):
    optimizer_strat.addStrategy(profitstrat_1_optimizer, {'from': accounts[1]})
    fund_with_deposit_and_hard_work_with_compound_strategy.doHardWork({'from':accounts[1]})
    assert optimizer_strat.getStrategies()[0] == ["CompoundLendingStrategyMainnetUSDC", compound_strat, compound_strat.investedUnderlyingBalance(), compound_strat.apr()]
...
def test_fund_with_compound_strategy_and_profit50_strategy_hardwork(fund_with_deposit_and_hard_work_with_compound_strategy, optimizer_strat, compound_strat, profitstrat_50_optimizer, accounts, usdc):
    deposited_amount = 1000 * (10 ** usdc.decimals())
...
@pytest.fixture
def fund_with_deposit_and_hard_work_with_compound_strategy_and_profit1_strategy_hardwork(fund_through_proxy_with_compound_strategy_and_deposit, optimizer_strat, compound_strat, accounts, profitstrat_1_optimizer):
    optimizer_strat.addStrategy(profitstrat_1_optimizer, {'from': accounts[1]})
    required_fund = fund_through_proxy_with_compound_strategy_and_deposit
    required_fund.doHardWork({'from':accounts[1]})
    return required_fund

def test_withdraw_small(fund_with_deposit_and_hard_work_with_compound_strategy_and_profit1_strategy_hardwork, compound_strat, usdc, test_usdc_account):    
    required_fund = fund_with_deposit_and_hard_work_with_compound_strategy_and_profit1_strategy_hardwork
...
def test_withdraw_large(fund_with_deposit_and_hard_work_with_compound_strategy_and_profit1_strategy_hardwork, compound_strat, usdc, test_usdc_account):
    required_fund = fund_with_deposit_and_hard_work_with_compound_strategy_and_profit1_strategy_hardwork
    ...
    assert float(strategy_balance_before - strategy_balance_after) == pytest.approx((amount_to_withdraw - token_in_fund_before), rel=1e-5)

def test_remove_strategies_from_optimizer(fund_with_deposit_and_hard_work_with_compound_strategy_and_profit1_strategy_hardwork, profitstrat_1_optimizer, compound_strat, optimizer_strat, accounts, zero_account):
    ...
    tx = optimizer_strat.removeStrategy(compound_strat, {'from': accounts[1]})
    ...
    assert float(strategy_balance_after) == pytest.approx(strategy_balance_before)
    assert tx.events["StrategyRemovedOptimizer"].values() == [compound_strat]
...
def test_remove_strategy_from_fund(fund_with_deposit_and_hard_work_with_compound_strategy_and_profit1_strategy_hardwork, optimizer_strat, usdc, accounts):
