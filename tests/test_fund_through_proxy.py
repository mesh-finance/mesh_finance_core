...
def test_initialization(fund_through_proxy, accounts, token):
    ...
    assert fund_through_proxy.relayer() == accounts[3]
...
def test_set_fund_manager_with_governance(fund_through_proxy, accounts):
    fund_through_proxy.setFundManager(accounts[2], {'from': accounts[0]})
...
def test_set_relayer(fund_through_proxy, accounts):
    fund_through_proxy.setRelayer(accounts[4], {'from': accounts[1]})
    
    assert fund_through_proxy.relayer() == accounts[4]

def test_set_relayer_with_random_account(fund_through_proxy, accounts):
    
    with brownie.reverts("Not fund manager"):
        fund_through_proxy.setRelayer(accounts[4], {'from': accounts[0]})

def test_set_deposit_limit(fund_through_proxy, accounts):
    fund_through_proxy.setDepositLimit(10, {'from': accounts[1]})
    
    assert fund_through_proxy.depositLimit() == 10

def test_set_deposit_limit_with_random_account(fund_through_proxy, accounts):
    
    with brownie.reverts("Not fund manager"):
        fund_through_proxy.setDepositLimit(10, {'from': accounts[0]})

def test_set_deposit_limit_per_tx_max(fund_through_proxy, accounts):
    fund_through_proxy.setDepositLimitTxMax(10, {'from': accounts[1]})
    
    assert fund_through_proxy.depositLimitTxMax() == 10

def test_set_deposit_limit_per_tx_max_with_random_account(fund_through_proxy, accounts):
...
def test_set_deposit_limit_per_tx_min(fund_through_proxy, accounts):
    fund_through_proxy.setDepositLimitTxMin(10, {'from': accounts[1]})
    
    assert fund_through_proxy.depositLimitTxMin() == 10

def test_set_deposit_limit_per_tx_max_less_than_min(fund_through_proxy, accounts):
    fund_through_proxy.setDepositLimitTxMin(10, {'from': accounts[1]})

    with brownie.reverts("Max limit greater than min limit"):
        fund_through_proxy.setDepositLimitTxMax(5, {'from': accounts[1]})

def test_set_deposit_limit_per_tx_min_greater_than_max(fund_through_proxy, accounts):
    fund_through_proxy.setDepositLimitTxMax(10, {'from': accounts[1]})

    with brownie.reverts("Min limit greater than max limit"):
        fund_through_proxy.setDepositLimitTxMin(15, {'from': accounts[1]})

def test_set_deposit_limit_per_tx_min_with_random_account(fund_through_proxy, accounts):
...
def test_set_performance_fee_fund_greater_than_max(fund_through_proxy, accounts):
    with brownie.reverts("Fee greater than max limit"):
        fund_through_proxy.setPerformanceFeeFund(5000, {'from': accounts[1]})

def test_set_performance_fee_fund_with_random_account(fund_through_proxy, accounts):
    with brownie.reverts("Not fund manager"):
...
def test_set_platform_fee_with_random_account(fund_through_proxy, accounts):
    with brownie.reverts("Not governance"):
        fund_through_proxy.setPlatformFee(100, {'from': accounts[1]})
